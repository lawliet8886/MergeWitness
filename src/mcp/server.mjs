...
const definitions = [
  { name: 'prepare', description: 'Create trusted, disposable Git snapshots and run existing tests.', inputSchema: { type: 'object', required: ['repoPath', 'baseRef', 'branchARef', 'branchBRef'], properties: { repoPath: { type: 'string' }, baseRef: { type: 'string' }, branchARef: { type: 'string' }, branchBRef: { type: 'string' }, testCommand: { type: 'array', items: { type: 'string' } } } } },
